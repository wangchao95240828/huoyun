    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http, BearerAuthFilter bearerAuthFilter) throws Exception {
        return http
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                // P0-D4 细粒度新增 (按业务模块 read/write/calc 三档):
                // 客户子表 (D2/D3)
                .requestMatchers(HttpMethod.POST, "/api/acc/customer-sites/**", "/api/acc/customer-contacts/**", "/api/acc/customer-salesmen/**", "/api/acc/customer-rate-cards/**", "/api/acc/customer-logins/**")
                    .hasAnyAuthority("customer.write", "operation.order.write", "ROLE_ADMIN")
                // HR 工资引擎 / 提成引擎 (D5/B3) - calculate 必须有专门权限
                .requestMatchers(HttpMethod.POST, "/api/acc/wages/calculate", "/api/acc/commissions/calculate")
                    .hasAnyAuthority("hr.wage.calculate", "admin.user.write", "ROLE_ADMIN")
                // 借款利息计提 (B4)
                .requestMatchers(HttpMethod.POST, "/api/acc/borrowings/*/accrue-interest")
                    .hasAnyAuthority("finance.account.write", "ROLE_ADMIN")
                // 配载同步承运商 (C5) - 高风险, 严格权限
                .requestMatchers(HttpMethod.POST, "/api/acc/stowages/*/sync-carrier")
                    .hasAnyAuthority("carrier.sync", "warehouse.stowage.audit", "ROLE_ADMIN")
                // 追踪轮询 (poll-ups-batch 等)
                .requestMatchers(HttpMethod.POST, "/api/acc/tracking/**", "/api/acc/orders/*/poll-tracking-batch")
                    .hasAnyAuthority("carrier.track.poll", "operation.order.write", "ROLE_ADMIN")
                // AI 客服 (低风险, 读写都 customer.service)
                .requestMatchers("/api/acc/ai-cs/**")
                    .hasAnyAuthority("customer.service", "admin.user.write", "ROLE_ADMIN")
                // GET 读权限细化 (之前 anyRequest authenticated 太宽)
                .requestMatchers(HttpMethod.GET, "/api/acc/charges/**", "/api/acc/bills/**", "/api/acc/receiveds/**", "/api/acc/profits/**")
                    .hasAnyAuthority("finance.receivable.read", "finance.receivable.write", "ROLE_ADMIN")
                .requestMatchers(HttpMethod.GET, "/api/acc/costs/**", "/api/acc/payments/**", "/api/acc/supplier-payables/**")
                    .hasAnyAuthority("finance.payable.read", "finance.payable.write", "ROLE_ADMIN")
                .requestMatchers(HttpMethod.GET, "/api/acc/wages/**", "/api/acc/borrowings/**", "/api/acc/employees/**", "/api/acc/socials/**", "/api/acc/funds/**", "/api/acc/attendances/**", "/api/acc/commissions/**")
                    .hasAnyAuthority("hr.read", "admin.user.read", "admin.user.write", "ROLE_ADMIN")
